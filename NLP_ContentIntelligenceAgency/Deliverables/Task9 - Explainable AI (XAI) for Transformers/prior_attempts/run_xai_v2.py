...
from local_transformers.modeling_roberta_lrp import RobertaForSequenceClassificationWithLRP
...
tokenizer = AutoTokenizer.from_pretrained("../../distilroberta_finetuned_v2")



# Load data
df = pd.read_parquet("../../group_combined_no_features.parquet")
df = df[["text", "general_emotion"]].iloc[-1044:].reset_index(drop=True)

# Разделяем на train/test. (Аналог train_test_split)
from sklearn.model_selection import train_test_split

train_df, test_df = train_test_split(df, test_size=0.2, random_state=42)

# Теперь превращаем в torch Dataset
class MyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Convert to dict for your collator
        # Note: sst code expects features like "input_ids", etc.
        features = {
...
train_dataset = MyDataset(train_df["text"].tolist(), train_df["general_emotion"].tolist(), tokenizer)
test_dataset  = MyDataset(test_df["text"].tolist(), test_df["general_emotion"].tolist(), tokenizer)
...
model = RobertaForSequenceClassificationWithLRP.from_pretrained("../../distilroberta_finetuned_v2")
model.roberta.embeddings.requires_grad = False
for name, param in model.named_parameters():                
    if name.startswith('embeddings'):
        param.requires_grad = False
        
pretrained_embeds = model.roberta.embeddings

params = torch.load('model.pt', map_location=torch.device(device))
