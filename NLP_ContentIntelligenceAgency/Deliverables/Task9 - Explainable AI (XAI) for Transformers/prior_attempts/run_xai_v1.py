...
le = LabelEncoder()
df["label_int"] = le.fit_transform(df["general_emotion"])

train_df, test_df = train_test_split(df, test_size=0.2, random_state=42)

# Now turn it into a torch Dataset
class MyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Convert to dict for the collator
        features = {
...
train_dataset = MyDataset(train_df["text"].tolist(), train_df["label_int"].tolist(), tokenizer)
test_dataset  = MyDataset(test_df["text"].tolist(), test_df["label_int"].tolist(), tokenizer)
...
params = torch.load("sst2-3layer-model.pt", map_location=device)
params.pop("embeds.position_ids", None)
...
for flip_case in [ 'generate' , 'pruning']:
    ...
    for case, random_order in [('random', True), 
        ...
        for x in test_data_loader:
            arr = np.array(x['input_ids'])  # shape (12,1)
            arr = np.squeeze(arr)           # shape (12,)
            input_ids = torch.tensor(arr, dtype=torch.float32, requires_grad = True).unsqueeze(0).long().to(device)  # shape (1,12)
